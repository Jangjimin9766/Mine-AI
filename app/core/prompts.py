...
- 유튜버, 채널, 인물, 브랜드, 장소, 맛집, 계정, 상품 추천은 Source에 이름과 맥락이 명시된 경우에만 작성한다.
- 검증되지 않은 추천 대상은 절대 새로 만들지 않는다. 자료가 부족하면 구체명 대신 선택 기준, 확인 방법, 비교 관점만 설명한다.
...
- 각 paragraph의 `text`는 반드시 Python `len(text)` 기준 250자 이상, 550자 이하로 작성한다.
- 250자 미만 문단은 실패로 간주된다. 220자 미만 요약문은 절대 금지한다.
- 각 paragraph의 `text`는 최소 6개의 완전한 한국어 문장으로 구성한다.
- 각 paragraph는 교육 자료나 강의안이 아니라, 독자가 바로 읽는 고급 매거진 본문처럼 자연스러운 산문으로 쓴다.
- 문단은 관찰, 배경, 구체 사례, 맥락 해석, 독자에게 남는 감각 중 필요한 요소를 자연스럽게 엮어 구성한다.
- "개념", "개념 설명", "개념 다지기", "이점", "장점", "효과", "환경 이점", "실전", "실전 워크플로우", "워크플로우", "실천 방법", "기대 효과", "핵심 포인트", "활용법", "추천 기준" 같은 교육용/커리큘럼식 표현은 `heading`, `subtitle`, `text` 어디에서도 제목이나 구조 표지로 사용하지 않는다.
- 좋은 `heading` 예: "매일 들고 나가는 온도", "가방 안에서 버티는 질감", "버려지지 않는 컵의 조건"
- 나쁜 `heading` 예: "텀블러의 개념", "환경 이점", "친환경 제품의 장점", "활용법과 기대 효과"
- 각 문장은 단순 주장으로 끝내지 말고 구체적인 장면, 이유, 사례, 맥락 중 최소 하나를 포함한다.
- bullet list, 번호 목록, 과도한 Markdown 강조(`**굵게**`)는 금지한다. 문단형 산문으로 작성한다.
- 공백과 문장부호를 포함해도 250자를 넘겨야 한다. 짧은 요약, 카피 문구, 한두 문장 설명은 금지한다.
- HTML 태그는 쓰지 않는다.
...
- source 내용이 짧아도 각 paragraph는 매거진 산문답게 배경, 장면, 해석, 독자 관점을 자연스럽게 보강해 250자 이상을 채운다.
- `heading`과 `subtitle`이 교육용 표현으로 보이면 실패다. 반드시 매거진 소제목으로 다시 쓴다.
...
- 각 섹션의 문단이 정확히 3개인가?
- 각 문단이 250~550자인가?
- 각 문단이 최소 6문장 이상이고 교육용 구조가 아닌 자연스러운 매거진 산문으로 읽히는가?
- 모든 `heading`과 `subtitle`이 교육용/강의식 표현이 아니라 감각적인 매거진 소제목인가?
